...
exports.genre_delete_post = function(req, res, next) {
    ...
    async.parallel({
        ...
        genres_books: function(callback) {
          Book.find({ 'genre': req.body.genrerid }).exec(callback)
        },
    }, function(err, results) {
        if (err) { return next(err); }
        // Success
...
exports.genre_update_get = function(req, res, next) {
    ...
    .exec(function(err, result) {
        if (err) { return next(err); }
        // Success, render genre update form.
        res.render('genre_form', { title: 'Update Genre: '+result.name, genre: result});
    });
};
...
exports.genre_update_post = [
    ...
    (req, res, next) => {
        ...
        if (!errors.isEmpty()) {
            ...
            // Data from form is valid. Update genre object and redirect to genres page

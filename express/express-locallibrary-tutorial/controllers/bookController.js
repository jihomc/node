...
exports.index = function(req, res) {
    ...
    async.parallel({
        book_count: function(callback) {
            // Pass an empty object as match condition to find all
...
exports.book_delete_post = function(req, res, next) {
    ...
    async.parallel({
        ...
        if (results.book_instances.length > 0) {
            // Book has bookinstances. Render in the same way as GET route
            res.render('book_delete', { title: 'Delete Book', book: results.book, book_instances: results.book_instances });
        }
        else {
            // No bookinstances. Delete book and redirect to books page
            Book.findByIdAndRemove(req.body.bookid, function deleteAuthor(err) {
                if (err) { return next(err); }
                // Book deleted, redirect user to list of books
                res.redirect('/catalog/books');
            });
        }
    });
